...
function LocalAppStateProvider({ children }: { children: ReactNode }) {
  const [localState, setLocalState] = useState(() => defaultSyncedState());
  ...
  const value = useMemo<AppStateContextValue>(() => {
    ...
    async function setSettings(settings: Partial<Settings>) {
      setLocalState((prev) => normalizeState({ ...prev, ...settings }));
    }
    ...
    return {
      ...
      createScenario: async (options) => {
        setLocalState((prev) => {
          ...
          const sourceValues =
            current.scenarioValues[current.activeScenarioId] ??
            defaultScenarioValues;
          const scenario = {
            id: options.id,
            name: options.name,
          ...
          return normalizeState({
            ...current,
            scenarios: { ...current.scenarios, [options.id]: scenario },
            scenarioValues: {
              ...current.scenarioValues,
              [options.id]: options.duplicateFromActive
                ? structuredClone(sourceValues)
                : structuredClone(defaultScenarioValues),
            },
            activeScenarioId: options.id,
          });
        });
      },
      renameScenario: async (scenarioId, name) => {
        setLocalState((prev) => {
          const scenario = prev.scenarios[scenarioId];
      ...
      deleteScenario: async (scenarioId) => {
        setLocalState((prev) => {
          const current = normalizeState(prev);
          if (Object.keys(current.scenarios).length <= 1) return current;
          const scenarios = { ...current.scenarios };
          const scenarioValues = { ...current.scenarioValues };
          delete scenarios[scenarioId];
          delete scenarioValues[scenarioId];
          const activeScenarioId = scenarios[current.activeScenarioId]
            ? current.activeScenarioId
            : (Object.keys(scenarios)[0] ?? defaultScenarioId);
          return normalizeState({
            ...current,
            scenarios,
            scenarioValues,
            activeScenarioId,
            comparedScenarioIds: current.comparedScenarioIds.filter(
              (id) => id !== scenarioId,
            ),
          });
        });
      ...
      updateActiveScenarioValues: async (update) => {
        setLocalState((prev) => {
          const current = normalizeState(prev);
          const scenarioId = current.activeScenarioId;
          const values = normalizeScenarioValues(
            current.scenarioValues[scenarioId] ?? defaultScenarioValues,
          );
          const nextValues =
          ...
          return normalizeState({
            ...
            scenarioValues: {
              ...current.scenarioValues,
              [scenarioId]: normalizeScenarioValues(nextValues),
            },
          });
        });
      },
      setCredits: async (update) => {
        setLocalState((prev) => {
          ...
          const credits =
            typeof update === "function" ? update(values.credits) : update;
          return normalizeState({
            ...current,
            scenarioValues: {
              ...current.scenarioValues,
              [scenarioId]: normalizeScenarioValues({ ...values, credits }),
            },
          });
        });
      },
      ...
      createLiquidityScenario: async (options) => {
        setLocalState((prev) => {
          const current = normalizeState(prev);
          const sourceValues =
            current.liquidityScenarioValues[
              current.activeLiquidityScenarioId
            ] ?? defaultLiquidityScenarioValues;
          const scenario = {
            id: options.id,
            name: options.name,
          ...
          return normalizeState({
            ...
            activeLiquidityScenarioId: options.id,
          });
        });
      },
      renameLiquidityScenario: async (scenarioId, name) => {
        setLocalState((prev) => {
          const scenario = prev.liquidityScenarios[scenarioId];
          if (!scenario) return prev;
          return normalizeState({
            ...prev,
            liquidityScenarios: {
              ...prev.liquidityScenarios,
              [scenarioId]: { ...scenario, name },
            },
          });
        });
      ...
      deleteLiquidityScenarioById: async (scenarioId) => {
        setLocalState((prev) => {
          const current = normalizeState(prev);
          if (Object.keys(current.liquidityScenarios).length <= 1)
            return current;
          const liquidityScenarios = { ...current.liquidityScenarios };
          const liquidityScenarioValues = {
            ...current.liquidityScenarioValues,
          };
          delete liquidityScenarios[scenarioId];
          delete liquidityScenarioValues[scenarioId];
          const activeLiquidityScenarioId = liquidityScenarios[
            current.activeLiquidityScenarioId
          ]
            ? current.activeLiquidityScenarioId
            : (Object.keys(liquidityScenarios)[0] ??
              defaultLiquidityScenarioId);
          return normalizeState({
            ...current,
            liquidityScenarios,
      ...
      updateActiveLiquidityScenarioValues: async (update) => {
        setLocalState((prev) => {
          const current = normalizeState(prev);
          const scenarioId = current.activeLiquidityScenarioId;
          const values = normalizeLiquidityScenarioValues(
            current.liquidityScenarioValues[scenarioId] ??
              defaultLiquidityScenarioValues,
          );
          const nextValues =
            typeof update === "function" ? update(values) : update;
          return normalizeState({
            ...
            liquidityScenarioValues: {
              ...current.liquidityScenarioValues,
              [scenarioId]: normalizeLiquidityScenarioValues(nextValues),
            },
          });
        });
      },
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  if (process.env.NEXT_PUBLIC_E2E_LOCAL_STATE === "1") {
